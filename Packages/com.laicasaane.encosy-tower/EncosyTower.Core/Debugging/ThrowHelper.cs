...
{
    /// <summary>
    /// Provides exception helpers for Encosy Tower validation.
    /// </summary>
    /// <remarks>
    /// Calls to methods marked with <see cref="ConditionalAttribute"/> use the following block in
    ...
    /// #endif
    /// </code>
    /// Validation is enabled when any supported validation symbol is enabled, unless
    /// <c>DISABLE_ENCOSY_CHECKS</c> is enabled. Because <c>#define</c> is file-scoped, this block must
    /// be present in each source file that calls the conditional methods. Without
    /// <c>__ENCOSY_VALIDATION__</c>, the compiler omits those calls and their argument evaluation.
    /// </remarks>
